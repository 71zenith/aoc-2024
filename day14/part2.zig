...
pub fn main() !void {
    ...
    for (0..10000) |x| {
        const new = try vec.clone();
        vec.clearRetainingCapacity();
        const newgrid = try allocator.alloc([]u1, height);
        const file = try fs.cwd().createFile(try std.fmt.allocPrint(allocator, "mk/{d}.txt", .{x}), .{});
        defer file.close();
        for (newgrid) |*row| {
            row.* = try allocator.alloc(u1, width);
        }
        for (new.items) |o| {
            newgrid[@as(usize, @intCast(o.pos[1]))][@as(usize, @intCast(o.pos[0]))] = 1;
        }
        for (newgrid) |row| {
            for (row) |cell| {
                if (cell == 0) {
        ...
        for (new.items) |o| {
            ...
            try vec.append(info{ .pos = newpos, .vel = o.vel });
        }
    }
    // var q1: usize = 0;
    // var q2: usize = 0;
