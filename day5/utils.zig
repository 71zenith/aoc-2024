...
pub fn splitComma(data: []const u8) [][]const u8 {
    const line_count = count(u8, data, ',');
    var split = alloc([]const u8, line_count);
...
pub fn countStruct(comptime T: type, haystack: []const T, needle: T) usize {
    var total: usize = 0;

    for (haystack) |element| {
        if (element.l == needle.l and element.r == needle.r)
            total += 1;
    }
